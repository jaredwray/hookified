...
- [ ] `SECURITY.md` present — contact info + "How this repository is secured" summary (PR #175 pending)
- [ ] `DEFENSE_IN_DEPTH.md` present (this file) (PR #175 pending)
...
- [ ] 7-day cooldown: `minimumReleaseAge: 10080`, `minimumReleaseAgeStrict: true`, `minimumReleaseAgeIgnoreMissingTime: false`; no first-party `minimumReleaseAgeExclude` (PR #178 pending)
- [ ] `trustPolicy: no-downgrade`; no first-party `trustPolicyExclude` (PR #179 pending)
...
- [ ] Every action pinned to a full commit SHA (`npx actions-up`) (PR #180 pending)
- [ ] Every job installs Socket Firewall (`SocketDev/action` SHA-pinned, `firewall-version` pinned); `pnpm install` / `npm install` run as `sfw pnpm install` / `sfw npm install` (PR #181 pending)
- [ ] `.github/workflows/check-workflows.yaml` lints workflows with zizmor on every PR
- [ ] `persist-credentials: false` on checkouts that don't push (PR #182 pending)
- [x] No `pull_request_target` on workflows that run untrusted PR code — verified
- [ ] Artifact-publishing workflows disable `actions/setup-node` default caching (`package-manager-cache: false`) to prevent cache poisoning (PR #183 pending)
